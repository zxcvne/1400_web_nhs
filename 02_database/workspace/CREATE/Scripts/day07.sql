...
-- PK가 단일 컬럼이면 진부분집합이 없다
-- -> 2차 정규화할 대상 없음
-- 3차 정규화 대상 => typ_cd로 결정되는 typ_cd_nm
-- 3차 정규화 대상 => sido_cd로 결정되는 sido_cd_nm
-- 3차 정규화 대상 => siggu_cd로 결정되는 siggu_cd_nm

-- type_와 typ_cd_nm을 보관하는 
-- typ_cd_dtl을 만드시오.
drop table sido_cd_dtl;
drop table siggu_cd_dtl;
drop table typ_cd_dtl;
...
-- 강원도에 있는 상급 종합병원의 병원명, 시군구명, 주소를 구하시오.
-- bak 테이블을 이용하지 마시오.
