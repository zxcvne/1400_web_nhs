...
-- movie에서 genre가 Drama인 것만 movie2에 적재
desc movie2;
insert into movie2(movie_name) 
select movie_name
from   movie
-- where  genre = 'Drama'
;
...
-- 오늘 밤에 데이터 10억건을 넣어야 한다??
-- 매 row마다 인덱스에 추가 + PK(Constraint) 검사
-- 인덱스를 제거 -> 데이터 적재 -> 인덱스 재생성(rebuild)

-- index의 naming rule 규칙
...
-- 관객 등급이 있는 영화의 평균관람가 연령과 개수를 구하시오.
...
from   dual
;
select length('qoufghgoeiufieof') from dual;
select movie_name, length(movie_name) as "글자길이"
from   movie;  -- 한글은 3글자로 인식
...
select *
from   movie
where  substr(open_date, 1, 4) = '2024'
;
...
select movie_name
     ...
     , date_add(str_to_date(open_date, '%Y%m%d'), interval 100 hour)
from   movie
where  movie_name = '파묘'
;
